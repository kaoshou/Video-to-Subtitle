...
- 加強字幕編輯器與 EverCam 的檔案寫入安全，防止惡意符號連結將輸出導向其他檔案。
- 字幕原始碼預覽改用記憶體解析，儲存改用安全暫存與檔案替換。
- EverCam 原首頁備份失敗時停止部署；重複部署保留既有備份與額外自訂資源。
- 同步更新 Windows 字幕更新工具，新增 Windows／macOS 安全回歸測試作為發佈門檻。
- 遇到符號連結或 Windows junction／reparse point 時會拒絕相關操作；請先確認來源可信，再將所需檔案複製為一般檔案／資料夾。請勿直接刪除不明連結指向的目標。

## 🚀 v2.7.7 更新說明
本版本全面修復 macOS 平台上點擊「加入檔案...」崩潰閃退問題，徹底解決 Apple MLX 因著色器遺失退回 CPU 的問題，並實現免安裝外部 FFmpeg 的純原生 MLX 音訊解碼：
* **徹底修復 Apple MLX Metal GPU 加速與著色器同調部署 (Critical Fix)**：
  * **根因排查**：MLX 底層 `libmlx.dylib` 透過 `dladdr` 在其動態庫同級目錄中尋找 Metal 著色器，而 PyInstaller 打包後將動態庫置於 `Contents/MacOS/` 或 `Frameworks/`，與深層資料目錄脫鉤，引發 4 次 `library not found` 異常。
  * **全面修復**：在打包規範、CI/CD 流水線與程式啟動執行時三層全面進行 `mlx.metallib` 與 `default.metallib` 鏡像注入，並於 GitHub Actions 導入「主機環境隔離煙霧測試」，確保純淨 macOS 使用者無需任何設定與安裝，開箱即享 100% 滿血 Metal GPU 加速！

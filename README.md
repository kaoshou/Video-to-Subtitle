...
基於 OpenAI Whisper 模型的本地端桌面應用程式，幫它套上一層圖形介面，簡化 Whisper 的操作流程，使用者僅需進行簡單設定，即可完成影片轉換為字幕檔案（SRT、VTT）的作業。由於程式於本機端執行，無須將影音檔案上傳至雲端，可充分利用使用者電腦算力生成高品質字幕，兼具安全性、免費性與隱私保障，是一套安心可靠的語音轉字幕解決方案。

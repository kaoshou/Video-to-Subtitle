...
function Read-SubtitleCues {
    ...
    $content = $safeFiles.Read($File.Name)
...
if (-not ('SafeSubtitleFiles' -as [type])) {
    Add-Type -Path ([System.IO.Path]::Combine($PSScriptRoot, 'SafeSubtitleFiles.cs'))
}
$safeFiles = New-Object SafeSubtitleFiles($resolvedFolder)
try {
...
$safeFiles.Write('subtitles-data.js', $javascript)
